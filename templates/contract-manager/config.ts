 ...
 * Contract Manager - Intents Específicos
 * 
...
export interface contract-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const contract-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executecontract-managerIntent(
  intentId: keyof typeof contract-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = contract-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const contract-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executecontract-managerIntent('{{this.id}}', {
 ...
 * Contract Manager - Agreements Configuration
 * 
 ...
 */
export const contract-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createcontract-managerAgreement(
  type: keyof typeof contract-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = contract-managerAgreements[type];
  if (!agreement) {
