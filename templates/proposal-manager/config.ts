 ...
 * Proposal Manager - Intents Específicos
 * 
...
export interface proposal-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const proposal-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeproposal-managerIntent(
  intentId: keyof typeof proposal-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = proposal-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const proposal-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeproposal-managerIntent('{{this.id}}', {
 ...
 * Proposal Manager - Agreements Configuration
 * 
 ...
 */
export const proposal-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createproposal-managerAgreement(
  type: keyof typeof proposal-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = proposal-managerAgreements[type];
  if (!agreement) {
