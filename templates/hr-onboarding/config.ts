...
export interface hr-onboardingIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const hr-onboardingIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executehr-onboardingIntent(
  intentId: keyof typeof hr-onboardingIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = hr-onboardingIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const hr-onboardingIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executehr-onboardingIntent('{{this.id}}', {
 ...
 */
export const hr-onboardingAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createhr-onboardingAgreement(
  type: keyof typeof hr-onboardingAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = hr-onboardingAgreements[type];
  if (!agreement) {
