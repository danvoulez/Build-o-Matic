 ...
 * Knowledge Base - Intents Específicos
 * 
...
export interface knowledge-baseIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const knowledge-baseIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeknowledge-baseIntent(
  intentId: keyof typeof knowledge-baseIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = knowledge-baseIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const knowledge-baseIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeknowledge-baseIntent('{{this.id}}', {
 ...
 * Knowledge Base - Agreements Configuration
 * 
 ...
 */
export const knowledge-baseAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createknowledge-baseAgreement(
  type: keyof typeof knowledge-baseAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = knowledge-baseAgreements[type];
  if (!agreement) {
