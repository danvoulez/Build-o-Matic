...
export interface crm-liteIntent extends IntentRequest {
  intent: string;
 ...
 */
export async function executecrm-liteIntent(
  intentId: keyof typeof crm-liteIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = crm-liteIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const crm-liteIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executecrm-liteIntent('{{this.id}}', {
 ...
 * CRM Lite - Agreements Configuration
 * 
 ...
 */
export const crm-liteAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createcrm-liteAgreement(
  type: keyof typeof crm-liteAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = crm-liteAgreements[type];
  if (!agreement) {
