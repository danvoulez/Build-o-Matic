 ...
 * OKRs Manager - Intents Específicos
 * 
...
export interface okrs-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const okrs-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeokrs-managerIntent(
  intentId: keyof typeof okrs-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = okrs-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const okrs-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeokrs-managerIntent('{{this.id}}', {
 ...
 * OKRs Manager - Agreements Configuration
 * 
 ...
 */
export const okrs-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createokrs-managerAgreement(
  type: keyof typeof okrs-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = okrs-managerAgreements[type];
  if (!agreement) {
