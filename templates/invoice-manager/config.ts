...
export interface invoice-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const invoice-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeinvoice-managerIntent(
  intentId: keyof typeof invoice-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = invoice-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const invoice-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeinvoice-managerIntent('{{this.id}}', {
 ...
 */
export const invoice-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createinvoice-managerAgreement(
  type: keyof typeof invoice-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = invoice-managerAgreements[type];
  if (!agreement) {
