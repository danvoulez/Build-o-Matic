 ...
 * Asset Tracker - Intents Específicos
 * 
...
export interface asset-trackerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const asset-trackerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeasset-trackerIntent(
  intentId: keyof typeof asset-trackerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = asset-trackerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const asset-trackerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeasset-trackerIntent('{{this.id}}', {
 ...
 * Asset Tracker - Agreements Configuration
 * 
 ...
 */
export const asset-trackerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createasset-trackerAgreement(
  type: keyof typeof asset-trackerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = asset-trackerAgreements[type];
  if (!agreement) {
