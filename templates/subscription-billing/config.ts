    ...
    <div style={{ 
      ...
      <header style={{ 
        ...
        <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#1f2937' }}>
          Subscription Billing - {{companyName}}
        </h1>
 ...
 * Subscription Billing - Intents Específicos
 * 
...
export interface subscription-billingIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const subscription-billingIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executesubscription-billingIntent(
  intentId: keyof typeof subscription-billingIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = subscription-billingIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const subscription-billingIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executesubscription-billingIntent('{{this.id}}', {
 ...
 * Subscription Billing - Agreements Configuration
 * 
 ...
 */
export const subscription-billingAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createsubscription-billingAgreement(
  type: keyof typeof subscription-billingAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = subscription-billingAgreements[type];
  if (!agreement) {
