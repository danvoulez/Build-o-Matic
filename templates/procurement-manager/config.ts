    ...
    <div style={{ 
      ...
      <header style={{ 
        ...
        <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 'bold', color: '#1f2937' }}>
          Procurement Manager - {{companyName}}
        </h1>
 ...
 * Procurement Manager - Intents Específicos
 * 
...
export interface procurement-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const procurement-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeprocurement-managerIntent(
  intentId: keyof typeof procurement-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = procurement-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const procurement-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeprocurement-managerIntent('{{this.id}}', {
 ...
 * Procurement Manager - Agreements Configuration
 * 
 ...
 */
export const procurement-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createprocurement-managerAgreement(
  type: keyof typeof procurement-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = procurement-managerAgreements[type];
  if (!agreement) {
