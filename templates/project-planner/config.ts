 ...
 * Project Planner - Intents Específicos
 * 
...
export interface project-plannerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const project-plannerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeproject-plannerIntent(
  intentId: keyof typeof project-plannerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = project-plannerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const project-plannerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeproject-plannerIntent('{{this.id}}', {
 ...
 * Project Planner - Agreements Configuration
 * 
 ...
 */
export const project-plannerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createproject-plannerAgreement(
  type: keyof typeof project-plannerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = project-plannerAgreements[type];
  if (!agreement) {
