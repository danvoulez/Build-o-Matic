 ...
 * Expense Manager - Intents Específicos
 * 
...
export interface expense-managerIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const expense-managerIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executeexpense-managerIntent(
  intentId: keyof typeof expense-managerIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = expense-managerIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const expense-managerIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executeexpense-managerIntent('{{this.id}}', {
 ...
 * Expense Manager - Agreements Configuration
 * 
 ...
 */
export const expense-managerAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createexpense-managerAgreement(
  type: keyof typeof expense-managerAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = expense-managerAgreements[type];
  if (!agreement) {
