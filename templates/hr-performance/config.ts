 ...
 * HR Performance - Intents Específicos
 * 
...
export interface hr-performanceIntent extends IntentRequest {
  intent: string;
 ...
 * Intents disponíveis
 */
export const hr-performanceIntents = {
  {{#each intents}}
  /**
   * {{this.description}}
   * 
 ...
 */
export async function executehr-performanceIntent(
  intentId: keyof typeof hr-performanceIntents,
  payload: Record<string, any>
): Promise<any> {
  const intent = hr-performanceIntents[intentId];
  if (!intent) {
 ...
 * Type-safe helpers
 */
export const hr-performanceIntentHelpers = {
  {{#each intents}}
  {{#if this.helper}}
  /**
   * {{this.description}}
  ...
  {{this.helper.name}}: async ({{this.helper.params}}) => {
    return executehr-performanceIntent('{{this.id}}', {
 ...
 * HR Performance - Agreements Configuration
 * 
 ...
 */
export const hr-performanceAgreements: Record<string, AgreementDefinition> = {
  {{#each agreements}}
  /**
   * {{this.name}}
   * 
 ...
 */
export function createhr-performanceAgreement(
  type: keyof typeof hr-performanceAgreements,
  data: Record<string, any>
): AgreementDefinition {
  const agreement = hr-performanceAgreements[type];
  if (!agreement) {
