...
import { Deployment, ProvisionedResources, ToolPackage } from './types';
...
export class ArtifactDeployer {
  async prepare(tool: ToolPackage): Promise<Buffer> {
    // Simulate delta preparation (zstd not applied in MVP)
    const payload = JSON.stringify(tool, null, 2);
    return Buffer.from(payload);
  }

  async pushArtifacts(_tool: ToolPackage, prepared: Buffer): Promise<{ url: string; hash: string }> {
    const hash = crypto.createHash('sha256').update(prepared).digest('hex');
    // Stub CDN URL
    return { url: `https://cdn.ledger.dev/artifacts/${hash}.bin`, hash };
  }

  async deployToTarget(artifactUrl: string, resources: ProvisionedResources, config: Record<string, any>): Promise<Deployment> {
  ...
  async waitForReady(_deployment: Deployment): Promise<void> {
