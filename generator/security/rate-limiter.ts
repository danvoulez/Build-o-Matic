...
export class RateLimiter {
  ...
  constructor(
    ...
    maxRequests: number = 10, // 10 requisições por minuto
    blockDurationMs: number = 5 * 60 * 1000 // Bloqueio de 5 minutos
  ) {
   ...
   */
  isAllowed(identifier: string): { allowed: boolean; remaining: number; resetAt: number; error?: string } {
    const now = Date.now();
   ...
   */
  getStats(identifier: string): { count: number; remaining: number; resetAt: number; blocked: boolean } | null {
    const entry = this.limits.get(identifier);
    if (!entry) {
      return null;
    }
