...
import { selectLayout, LAYOUTS } from './layouts';
import { SecurityValidator } from './security/validator';
export class Customizer {
    async apply(template, answers) {
        ...
        const safeLayoutType = layoutValidation.sanitized;
    ...
    customizeAndTrim(source, answers) {
    ...
    replacePlaceholders(source, answers) {
        let out = source;
    ...
    applyFeatureBlocks(code, enabled) {
        return code.replace(/\/\/\s*FEATURE:([a-zA-Z0-9-_]+)\s*START[\s\S]*?\/\/\s*FEATURE:\1\s*END/g, (match, featureId) => (enabled.includes(featureId) ? match : ''));
    }
    applyIntegrationBlocks(code, enabled) {
        return code.replace(/\/\/\s*INTEGRATION:([a-zA-Z0-9-_]+)\s*START[\s\S]*?\/\/\s*INTEGRATION:\1\s*END/g, (match, integrationId) => (enabled.includes(integrationId) ? match : ''));
    }
    generateConfig(template, answers, theme) {
        const env = {
            NODE_ENV: 'production',
